-- Forward-only Builder content-contract normalisation.
-- Supersedes supabase/migrations_archived/20260521231500_builder_content_contract_normalisation.superseded.sql.
-- Forward-only, idempotent, non-destructive. No DELETE / DROP / TRUNCATE / RLS / type-gen changes.
-- Aligns live Builder content with the frontend contract: level values normalised to
-- secure / strong / top_band, unsupported active question families deactivated (is_active = false,
-- not deleted), and the three canonical Builder questions inserted-or-updated as active.
