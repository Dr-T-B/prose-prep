...
export function Component2DashboardMock() {
  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      {/* Top bar */}
      <header className="border-b border-rule bg-cream print:hidden">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8 space-y-10">
        ...
        <Section title="Theme Readiness" eyebrow="Comparative coverage">
          <div className="border border-rule overflow-hidden">
            <div className="grid grid-cols-[1fr_120px_120px_1.4fr] text-[10px] uppercase tracking-[0.2em] text-muted-ink bg-cream border-b border-rule">
            ...
            {THEMES.map((t) => (
              ...
              >
                ...
                <div className="px-4 py-3 text-muted-ink text-xs leading-relaxed">
                  {t.connection}
                </div>
              </div>
            ))}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Section title="Comparative Essay Practice" eyebrow="Build → Test → Refine">
            <div className="border border-rule">
              <div className="grid grid-cols-[60px_1fr_60px_50px] text-[10px] uppercase tracking-[0.2em] text-muted-ink bg-cream border-b border-rule">
              ...
              {ESSAYS.map((e) => (
                ...
                >
                  <div className="px-3 py-3 text-xs text-muted-ink">{e.date}</div>
                  <div className="px-3 py-3">
                    <div className="font-serif leading-snug">{e.question}</div>
                    <div className="text-[10px] uppercase tracking-wider text-muted-ink mt-1">
                      Weakest: {e.weakest}
                    </div>
                  </div>
            ...
            {WEAKNESSES.map((w) => (
              <li key={w.id} className="py-4 grid md:grid-cols-[100px_1fr_auto] gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-ink mt-1 leading-relaxed">
                    {w.detail}
                  </p>
                </div>
        ...
        >
          <div className="border-2 border-ink p-6 bg-paper space-y-6">
            <div className="flex items-baseline justify-between border-b border-rule pb-3">
              <div>
                <div className="text-[10px] uppercase tracking-[0.25em] text-muted-ink">
                  Edexcel · 9ET0/02
                </div>
              ...
              <ul className="space-y-3 text-sm">
                {PRINT_SUMMARY.keyQuotes.map((q, i) => (
                  <li key={i} className="grid grid-cols-[110px_1fr] gap-3">
                    <div className="text-[10px] uppercase tracking-wider text-muted-ink pt-1">
                      {q.text}
                    </div>
                    <div>
                      <div className="font-serif italic">"{q.quote}"</div>
                      <div className="text-xs text-muted-ink mt-1">{q.method}</div>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <SummaryBlock title="Context cues">
              <ul className="text-sm space-y-1 text-muted-ink">
            ...
            <SummaryBlock title="Paragraph checklist">
              <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-1 text-sm text-muted-ink list-disc pl-5">
      ...
      <footer className="border-t border-rule mt-10 print:hidden">
        <div className="mx-auto max-w-6xl px-6 py-5 text-xs text-muted-ink flex justify-between">
  ...
  return (
    <div className="border border-rule p-6 bg-cream">
      ...
      <div className="mt-5 pt-4 border-t border-rule flex items-baseline justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-ink">
            Weighted readiness
          </div>
...
function NextTaskPanel({
  ...
  return (
    <div className={`border border-ink p-6 bg-paper relative ${className}`}>
      <div className="absolute top-4 right-4 text-[10px] uppercase tracking-[0.25em] text-muted-ink">
        Recommended next
      </div>
      <div className="text-[10px] uppercase tracking-[0.25em] text-muted-ink">
      ...
      <p className="text-sm text-muted-ink mt-3 leading-relaxed">{task.reason}</p>
...
function AOCard({ ao }: { ao: AO }) {
  ...
  return (
    <div className="border border-rule p-4 bg-paper">
      ...
      <div className="text-[11px] text-muted-ink mt-1 leading-snug min-h-[28px]">
        {meta.label}
      </div>
      <div className="mt-3 flex items-baseline gap-2">
        ...
        <div
          className={`text-xs ${
            r.trend >= 0 ? "text-ink" : "text-muted-ink"
          }`}
        >
          {trendStr} this week
        </div>
      ...
      <p className="text-[11px] text-muted-ink mt-3 leading-snug">{r.note}</p>
    </div>
  );
}
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="border border-rule px-2 py-1 text-muted-ink">{children}</span>
  );
}
...
function SummaryBlock({
  ...
  return (
    <div>
      <div className="text-[10px] uppercase tracking-[0.25em] text-muted-ink mb-2">
        {title}
      </div>
      {children}
    </div>
